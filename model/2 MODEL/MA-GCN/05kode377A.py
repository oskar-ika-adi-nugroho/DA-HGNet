...
# ============================================================
# Velocity-only input A (Torch, GPU)
# Input:  x  [N, 3, T, V, M]
# Output: A  [N, 3, T, V, M]
# A[:, :, 1:-1] = (x[:, :, 0:-2] - x[:, :, 1:-1]) + (x[:, :, 2:] - x[:, :, 1:-1])
# ============================================================
class VelocityOnlyInput(nn.Module):
    def __init__(self):
        super().__init__()
...
# ============================================================
# Final Model: kode377 backbone, input is velocity-only A (torch)
# ============================================================
class Model(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # velocity-only input module
        self.velocity_only = VelocityOnlyInput()

        # channels stay 3 (but now they are velocity A)
    ...
    def forward(self, x, y):
        ...
        # 1) velocity-only A: [N,3,T,V,M]
        x = self.velocity_only(x)
